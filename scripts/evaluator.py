...
def _inference(
    ...
    """返回 (preds, trues, last_attn)，均为归一化空间。"""
    model.eval()
    preds, trues = [], []
    ...
    return np.concatenate(preds), np.concatenate(trues), attn_sample
...
def compute_metrics(true: NDArray, pred: NDArray) -> dict[str, float]:
    ...
    # Directional Accuracy: 连续时间步方向一致性
...
def main() -> None:
    ...
    # ❻ 汇总
    logger.info("=" * 60)
    logger.info("{:<20s} {:>8s} {:>8s} {:>8s} {:>8s}", "Model", "MSE", "RMSE", "MAPE%", "DA%")
    logger.info("-" * 60)
    for name, m in all_metrics.items():
        logger.info("{:<20s} {:>8.2f} {:>8.2f} {:>8.2f} {:>8.2f}",
                     _LABELS.get(name, name), m["MSE"], m["RMSE"], m["MAPE"], m["DA"])
    logger.info("=" * 60)
