...
def fetch_stock_data(symbol: str, years: int = 3) -> pd.DataFrame:
    ...
    try:
        ...
        if symbol.startswith("sh") or symbol.startswith("sz"):
            ...
            symbol_code = symbol[2:] if (symbol.startswith("sh") or symbol.startswith("sz")) else symbol
            df = ak.stock_zh_a_hist(symbol=symbol_code, period="daily", start_date=start_date, end_date=end_date)
        ...
        # 数据类型转换
        result = df[required_cols].astype(float)
...
def process_all_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available_cols = [col for col in FEATURE_COLS if col in final_df.columns]
    missing_cols = [col for col in FEATURE_COLS if col not in final_df.columns]
    
    if missing_cols:
        # 为缺失列填充0（通常是数据不足导致的特征缺失）
        for col in missing_cols:
            final_df[col] = 0.0
        st.warning(f"特征缺失，已填充0值: {missing_cols}")
...
def plot_candlestick_with_indicators(df: pd.DataFrame, title: str = "K线图与技术指标"):
    ...
    # 布林带
    if 'bbm_20' in df.columns and 'bbu_20' in df.columns and 'bbl_20' in df.columns:
        fig.add_trace(
            go.Scatter(x=df.index, y=df['bbu_20'], name="BB Upper", line=dict(color='gray', width=1, dash='dash')),
            row=1, col=1
        )
        fig.add_trace(
            go.Scatter(x=df.index, y=df['bbm_20'], name="BB Middle", line=dict(color='gray', width=1)),
            row=1, col=1
        )
        fig.add_trace(
            go.Scatter(x=df.index, y=df['bbl_20'], name="BB Lower", line=dict(color='gray', width=1, dash='dash'), fill='tonexty'),
            row=1, col=1
        )
...
def main():
    ...
    if st.sidebar.button("使用示例数据"):
        with st.spinner("生成示例数据..."):
            ...
            np.random.seed(42)
            prices = 3000 + np.cumsum(np.random.randn(len(dates)) * 20)
    ...
    if st.session_state.raw_data is not None:
        ...
        if len(selected_models) > 0:
            ...
            test_data = feature_data.iloc[-(lookback_days + 60):]
            
            # 创建缩放器
            feature_scaler, target_scaler = prepare_scalers(feature_data)
            ...
            for idx, model_name in enumerate(selected_models):
                ...
                predictions = sliding_window_inference(
                    model, feature_scaler, target_scaler, test_data, seq_len=60
                )
                predictions_dict[model_name] = predictions
                
                # 计算指标
                if len(predictions) > 0:
                    true_values = test_data['close'].iloc[60:].values  # 对齐预测值
            ...
            fig_comparison = plot_predictions_comparison(
                feature_data, predictions_dict, lookback_days
            )
        ...
        with prediction_container:
            ...
            with col1:
                ...
                if st.button("预测下一个交易日收盘价", type="primary", use_container_width=True):
                    ...
                    # 获取最新数据
                    latest_data = feature_data.tail(60)
                    ...
                    if "LSTM_Transformer" in selected_models:
                        ...
                        # 准备输入
                        # 使用固化的特征列顺序，包含close作为输入特征之一
                        feature_cols = FEATURE_COLS
                        X_scaled = feature_scaler.transform(latest_data[feature_cols])
        ...
        1. **侧边栏配置**：选择预测标的和回测天数
        2. **选择对比模型**：勾选要参与对比的模型
        3. **获取数据**：点击"获取最新数据"按钮或使用示例数据
        4. **查看特征**：在模块一查看K线图和特征数据
        5. **模型对比**：在模块二查看各模型预测结果和性能指标
        6. **明日预测**：在模块三进行单步实时预测
        ...
        # 快速开始按钮
        if st.button("🚀 快速开始（使用示例数据）", type="primary"):
            with st.spinner("生成示例数据..."):
                dates = pd.date_range(end=datetime.now(), periods=365*3, freq='B')
                np.random.seed(42)
                prices = 3000 + np.cumsum(np.random.randn(len(dates)) * 20)
