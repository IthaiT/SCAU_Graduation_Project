...
为了清晰地呈现从基础理论研究、混合模型设计、对比实验分析到最终工程系统落地的完整过程，本文的行文结构安排如下：

第一章，绪论。主要交代本文的选题背景与实际意义，梳理目前国内外在时序预测及混合模型领域的研究现状，明确本文的核心研究内容与主要贡献，并给出全文的组织架构。

第二章，相关理论与技术基础。回顾并梳理时间序列预测的基础概念，详细剖析长短期记忆网络（LSTM）处理序列数据的门控机理，以及 Transformer 架构中自注意力机制（Self-Attention）的核心逻辑。此外，列举了用于后续实验检验的模型评价指标，为后文的算法设计提供理论支撑。

第三章，多维量价特征工程与数据处理。针对原始金融时序数据存在噪声大、维度单一的问题，详细说明实验数据的获取途径与基础面貌。在此基础上，构造衍生技术指标与时间周期编码等特征，并通过归一化操作与时序滑动窗口切分技术，完成模型输入前的数据预处理工作。

第四章，LSTM-Transformer 融合预测模型设计。本章为全文的算法核心。详细阐述所提出的混合网络整体架构，分别讲解如何利用 LSTM 模块提取时序的局部连续性特征，以及如何通过 Transformer 模块捕捉全局的长程依赖关系，最后明确前向传播流程与模型训练策略。

第五章，实验结果与性能分析。交代本次实验的软硬件环境与参数设定。在同等条件下，将本文构建的混合模型与单一的 LSTM、单一 Transformer 模型进行对比测试，通过直观的预测误差指标与拟合曲线，客观分析混合架构在不同场景下的预测性能表现。

第六章，基于 Streamlit 的量价预测可视化系统实现。结合计算机专业的工程实践要求，脱离纯代码环境，对前文表现最优的深度学习模型进行工程化封装。利用 Python 的 Streamlit 框架开发前端交互界面，实现从输入股票代码、历史数据渲染到未来趋势预测的可视化展示，完成算法的前端落地。

第七章，总结与展望。对整个实验期间的理论研究与系统开发工作进行全面回顾，总结实验结论。同时，客观分析当前模型与系统存在的局限性，并结合实际应用需求，对未来的改进与拓展方向做出规划。

\section{相关理论与技术基础}
\subsection{时间序列预测基本理论}

时间序列是指按时间顺序排列的一组观测数据点，通常记录了某个变量在不同时间点上的取值。这类数据广泛存在于金融、气象、工业监控等领域，具有明显的时间依赖性和连续性特征。时间序列分析的核心任务之一是根据历史数据预测未来趋势，这对于决策制定和风险管理具有重要意义。

时间序列数据通常包含几个关键特征：趋势性反映了数据长期的上升或下降方向；周期性表现为在固定时间间隔内重复出现的规律波动；季节性则与自然周期（如季节更替）相关联；此外，序列中还常常包含随机噪声成分。这些特征相互交织，使得准确预测变得具有挑战性。

在时间序列预测的发展过程中，传统的统计方法通常基于线性假设和固定参数模型。这类方法在处理简单线性关系时能够获得较好效果，但面对金融市场等复杂场景中普遍存在的非线性、非平稳性数据时，其建模能力受到限制。非线性意味着变量间的关系无法用简单线性函数描述，非平稳性则指数据的统计特性随时间发生变化，这都给传统预测方法带来了困难。

深度学习的兴起为时间序列预测提供了新的技术路径。与传统方法相比，神经网络模型不依赖于严格的统计假设，能够通过多层非线性变换自动学习数据中的复杂模式。特别是对于金融时间序列这类具有高度非线性特征的数据，深度学习模型能够更有效地捕捉价格与成交量之间的动态关联，在处理复杂时序数据方面展现出明显优势。这种数据驱动的方法为提升预测精度提供了新的可能性。

\subsection{长短期记忆网络（LSTM）原理}

循环神经网络（RNN）是早期处理序列数据的常用模型，但其在训练长序列时容易面临梯度消失问题，导致难以学习到长距离的时间依赖。为解决这一问题，长短期记忆网络（LSTM）被提出，通过引入门控机制来控制信息在不同时间步之间的传递。

LSTM的内部细胞结构如\ref{fig:lstm_cell}图所示。该结构的核心设计是细胞状态（Cell State），它作为信息传递的主干，能够在整个序列处理过程中保持相对稳定，缓解梯度消失问题。细胞状态的信息流由三个门控单元共同调节。
...
遗忘门决定细胞状态中哪些信息需要被丢弃。其计算过程如下：
\begin{equation}
f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)
\end{equation}
其中 $f_t$ 是遗忘门输出，$\sigma$ 表示sigmoid激活函数，$W_f$ 和 $b_f$ 分别为遗忘门的权重矩阵与偏置项，$h_{t-1}$ 是上一时间步的隐藏状态，$x_t$ 是当前输入。

输入门控制哪些新信息将被存入细胞状态。输入门包含两个部分：
\begin{equation}
...
\end{equation}
这里 $i_t$ 决定信息的更新程度，$\tilde{C}_t$ 表示候选的新细胞状态，$\tanh$ 是双曲正切激活函数。
...
\end{equation}
输出 $h_t$ 既作为当前时间步的输出，也作为下一时间步的隐藏状态输入。通过这种门控机制，LSTM能够有选择性地记住或遗忘信息，从而有效处理长序列数据。

\subsection{Transformer与自注意力机制（Self-Attention）}

Transformer架构最初应用于机器翻译任务，其核心创新在于完全基于注意力机制构建，摒弃了传统的循环结构。这种设计带来了两个主要优势：一是实现了序列处理的完全并行计算，大幅提高了训练效率；二是通过注意力机制直接建模序列中任意两个位置之间的依赖关系，有效解决了长距离依赖问题。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{Transformer.pdf}
\caption{Transformer 自注意力结构图}
\label{fig:attention}
\end{figure}

\FloatBarrier
自注意力机制的原理如图\ref{fig:attention}所示。 其基本思想是让序列中的每个位置都能够关注到序列所有位置的信息，并根据相关性进行加权聚合。首先，对于输入序列 $X$，通过三个不同的线性变换得到查询矩阵 $Q$、键矩阵 $K$ 和值矩阵 $V$：
\begin{equation}
...
\end{equation}
这里 $d_k$ 是键向量的维度，缩放因子 $\sqrt{d_k}$ 的作用是防止点积结果过大导致softmax函数梯度消失。

为了提高模型表示能力，Transformer采用了多头注意力机制。多个注意力头并行工作，各自学习不同的注意力模式：
\begin{equation}
...
\end{equation}
每个注意力头使用不同的参数矩阵 $W_i^Q, W_i^K, W_i^V$ 进行投影，最终的输出通过线性变换 $W^O$ 进行整合。多头注意力机制允许模型同时关注来自不同表示子空间的信息。

\subsection{深度学习模型评价指标}

在时间序列预测任务中，需要采用多个评价指标从不同角度衡量模型的预测性能。本文使用以下五个常用指标进行评估。

均方误差（Mean Squared Error, MSE）衡量预测值与真实值之间差异的平方的平均值：
\begin{equation}
\text{MSE} = \frac{1}{n} \sum_{i=1}^{n} (y_i - \hat{y}_i)^2
\end{equation}
MSE对较大误差赋予更高的权重，因此对异常值比较敏感。其数值越小表示预测精度越高。

均方根误差（Root Mean Squared Error, RMSE）是MSE的平方根：
\begin{equation}
\text{RMSE} = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (y_i - \hat{y}_i)^2}
\end{equation}
RMSE与原始数据的量纲一致，更易于直观解释预测误差的大小。

平均绝对误差（Mean Absolute Error, MAE）计算预测误差绝对值的平均值：
\begin{equation}
\text{MAE} = \frac{1}{n} \sum_{i=1}^{n} |y_i - \hat{y}_i|
\end{equation}
与MSE相比，MAE对异常值的敏感性较低，提供了更稳健的误差估计。

平均绝对百分比误差（Mean Absolute Percentage Error, MAPE）以百分比形式表示预测误差：
\begin{equation}
\text{MAPE} = \frac{100\%}{n} \sum_{i=1}^{n} \left|\frac{y_i - \hat{y}_i}{y_i}\right|
\end{equation}
MAPE提供了相对误差度量，便于在不同量级的数据集之间进行比较。但当真实值 $y_i$ 接近零时，MAPE可能趋于无穷大。

决定系数（R-squared, R²）反映了模型对数据变异的解释程度：
\begin{equation}
R^2 = 1 - \frac{\sum_{i=1}^{n} (y_i - \hat{y}_i)^2}{\sum_{i=1}^{n} (y_i - \bar{y})^2}
\end{equation}
其中 $\bar{y}$ 是真实值的平均值。R²的取值范围在0到1之间，数值越接近1表示模型拟合效果越好。该指标从整体上评估了模型捕捉数据变异的能力。

这些指标各有侧重：MSE和RMSE强调对较大误差的惩罚，MAE提供稳健的误差估计，MAPE关注相对误差，R²则从整体上评估模型的拟合优度。综合使用多个指标能够全面评价模型的预测性能。


\section{多维量价特征工程与数据处理}

\subsection{实验数据来源与说明}
沪深300指数（CSI 300）由上海和深圳证券交易所中市值大、流动性好的300只股票组成，综合反映了中国A股市场的整体表现，具有较好的代表性与数据连续性。实验选取该指数作为研究对象，日频行情数据的时间跨度为2006年3月1日至2025年12月31日。

原始行情数据通过 \texttt{akshare} 接口从新浪源获取，包含日期（\texttt{date}）、开盘价（\texttt{open}）、最高价（\texttt{high}）、最低价（\texttt{low}）、收盘价（\texttt{close}）、成交量（\texttt{volume}）等字段。由于新浪源未提供成交额（\texttt{amount}）字段，实验中采用“收盘价 $\times$ 成交量”进行合理近似计算。

金融时间序列常因市场节假日、休市等原因产生缺失值。为维持时间序列的连续性，实验采用金融数据分析中广泛使用的前向填充（Forward Fill, ffill）方法进行处理：若某交易日数据缺失，则使用前一个有效交易日的数值进行填充。此方法假设市场在非交易期间状态未发生突变，是处理金融日历数据缺失的常规做法。

\subsection{基础量价数据分析}
原始行情数据记录了每个交易日的开盘价、最高价、最低价、收盘价、成交量与成交额，这些基础量价变量构成特征工程的第一层。其中，收盘价是金融分析中最常使用的基准价格，成交量表征市场的活跃程度与流动性。量价关系的变化模式蕴含着供需关系、投资者情绪与资金流向等信息。

如图 \ref{fig:close_price} 所示，沪深300指数在考察期内呈现出典型的牛熊周期交替特征。然而，原始数据维度有限且噪声较高，直接用于复杂模型预测效果有限，因此需要构建更具判别性的衍生特征。

\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{price_trend.pdf}
...
\subsubsection{技术指标特征}
技术指标通过对历史价格和成交量进行数学变换，用于识别趋势、动量和波动率等模式。实验选取了移动平均线、相对强弱指数、平滑异同移动平均线以及布林带四类经典指标，使用 \texttt{pandas\_ta} 库进行批量计算，并在计算后移除了产生的NaN值。

\paragraph{移动平均线}
移动平均线通过平滑价格序列来识别趋势方向。本实验计算了短期（5日）和中期（20日）简单移动平均线（SMA），其计算公式为
\begin{equation}
  \text{SMA}_t(N) = \frac{1}{N} \sum_{i=0}^{N-1} \text{Close}_{t-i},
\end{equation}
其中 $\text{SMA}_t(5)$ 和 $\text{SMA}_t(20)$ 分别反映了短期和中期市场平均成本，两者的相互位置与价格的关系可用于判断趋势强度与潜在反转。

\paragraph{相对强弱指数}
相对强弱指数（RSI）通过比较特定周期内价格上涨与下跌的幅度来衡量资产的超买或超卖状态。计算公式为
\begin{equation}
...
\end{equation}
实验采用14日周期（$N=14$）。RSI值在0到100之间，通常认为高于70表明超买，低于30表明超卖，能够捕捉价格动量的衰竭点。

\paragraph{平滑异同移动平均线}
平滑异同移动平均线（MACD）用于识别趋势的变化、方向、动量和持续时间。它由差离值（DIF）、信号线（DEA）以及MACD柱状图三部分组成：
\begin{equation}
...
\end{equation}
实验采用标准参数（12, 26, 9），其中DIF为短期EMA与长期EMA之差，DEA为DIF的9日指数移动平均，MACD柱状图则为DIF与DEA之差。MACD的交叉与柱状图缩放能有效揭示趋势的加速与减速。

\paragraph{布林带}
布林带由一条中轨（$N$日SMA）和两条上下轨（中轨加减$N$倍标准差）组成，用于衡量价格波动率和识别极端价格。计算公式为
\begin{equation}
...
其中实验采用 $N=20$, $k=2$ 的标准参数。此外，还计算了带宽（$\text{BB\_width}$，反映波动率）和百分比（$\text{BB\_\%B}$，反映价格在通道内的相对位置）作为特征。

\subsubsection{宏观资金与跨市场特征}
为引入更广泛的市场驱动信息，实验模拟引入了北向资金动量作为代表外部资金动向的特征。北向资金数据基于东方财富接口获取，由于早期数据缺失，采用零填充处理。基于北向资金净流入计算其5日与20日移动平均，取二者差值作为北向资金动量特征。在实际应用中，此类特征可扩展为国债收益率、汇率、大宗商品价格等宏观变量。

\subsubsection{时间周期编码}
金融市场存在显著的日历效应，如“周内效应”和“月份效应”。为让模型能够捕捉这种周期性规律，实验对时间特征进行了连续性编码。

传统的整数编码（如周一=0，周日=6）会引入边界不连续性：周日（6）与周一（0）在时间上是连续的，但在数值空间上距离最大。为解决此问题，采用正弦和余弦函数将一维离散时间点映射到二维单位圆上。设原始时间特征为 $t$（星期几 $d \in [0,6]$，月份 $m \in [1,12]$），其周期长度为 $T$（$T_{\text{week}}=7$, $T_{\text{month}}=12$），则编码为
\begin{equation}
...
\subsection{数据预处理}
在完成特征构建后，需经过严格的预处理流程才能将数据转化为适用于深度学习模型训练的样本。

\subsubsection{归一化处理}
深度学习模型对输入特征的尺度敏感。实验采用最小-最大归一化（Min-Max Scaling）将特征映射到[0, 1]区间，公式为
\begin{equation}
  X' = \frac{X - X_{\min}}{X_{\max} - X_{\min}}.
\end{equation}
为防止前视偏差，归一化参数的拟合必须严格遵守时间顺序。实验按时间顺序将数据划分为训练集（72\%）、验证集（10\%）和测试集（18\%）。归一化操作所需的最小值（$X_{\min}$）和最大值（$X_{\max}$）仅从训练集数据中计算得到。然后，使用训练集上拟合好的缩放器分别对验证集和测试集进行变换。这意味着模型在训练和评估的任一阶段，都未曾接触到未来数据的全局分布信息。

目标变量（收盘价 \texttt{close}）同样进行独立的归一化，并保存其缩放器参数，以便在模型预测完成后，将预测值逆变换回原始价格尺度，用于计算最终的评估指标（如RMSE, MAPE）。

\subsubsection{时序滑动窗口切分}
LSTM与Transformer等模型要求输入为固定长度的序列。实验采用滑动窗口（Sliding Window）机制将一维时间序列转化为模型所需的样本对。如图 \ref{fig:sliding_window} 所示，给定总特征序列，设定历史窗口长度（Look-back window）$L_{\text{seq}}=60$，预测窗口长度 $L_{\text{pred}}=1$。对于时间点 $t$，构建一个输入-输出对 $(X_t, y_t)$：输入 $X_t$ 为一个形状为 $(L_{\text{seq}}, \text{num\_features})$ 的矩阵，包含从 $t-L_{\text{seq}}+1$ 到 $t$ 时刻的所有特征；输出 $y_t$ 为一个标量，即 $t+1$ 时刻的目标值（归一化后的收盘价）。
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{sliding_window.pdf}
\caption{时序滑动窗口切分机制示意图}
\label{fig:sliding_window}
\end{figure}

\FloatBarrier

随后，窗口以步长 $\text{step}=1$ 向后滑动，生成下一个样本对 $(X_{t+1}, y_{t+1})$。此过程遍历整个数据集（需跳过起始和结尾的边界）。最终，所有样本被堆叠成两个三维张量：输入张量 $X$ 形状为 $(N, L_{\text{seq}}, \text{num\_features})$，输出张量 $y$ 形状为 $(N, L_{\text{pred}})$，其中 $N$ 为总样本数。此滑动窗口样本构建方法在训练集、验证集和测试集内部独立进行，并确保了时间顺序的严格性，为时序预测模型提供了结构化的输入。
...
\end{itemize}
\subsection{模型训练策略}
为了在 LSTM、Transformer 以及 LSTM-Transformer 混合架构之间进行严谨且公平的性能评估，本文设计了一套基于贝叶斯优化的自动化超参数搜索（Hyperparameter Optimization, HPO）机制。不同神经网络架构对超参数的敏感度差异显著（例如，Transformer 极易受学习率和批次大小影响），若采用统一的固定超参数，将引入严重的评估偏见。因此，本文采用 Optuna 框架对各模型进行彻底的超参数解耦与独立寻优，确保最终性能对比建立在各架构的最佳状态之上。

\textbf{1. 公平性约束与搜索策略} \\
为保证对比实验的公平性，本文确立了以下搜索约束原则：
\begin{itemize}
...
\textbf{4. 计算资源优化与早退剪枝} \\
为提升 HPO 的搜索效率，本文引入了中位值剪枝算法（Median Pruner）。该算法在初始的 5 次预热试验和每个试验的前 10 步评估后激活，自动中止那些表现处于历史下半区的劣质试验，大幅节约了计算算力。此外，针对大模型（如高维度多层的并行混合架构）在极端超参数组合下可能引发的显存溢出（Out-Of-Memory, OOM）问题，系统设计了异常捕获机制，自动舍弃导致 OOM 的超参数组合并执行显存回收（Garbage Collection），保障了多模型并发搜索的稳定性。
