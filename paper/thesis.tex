...
在早期的序列建模研究中，循环神经网络（RNN）占据主导地位，但其存在的梯度消失问题严重限制了对长序列的建模能力。为此，长短期记忆网络（LSTM）被提出，通过引入门控机制有效解决了长序列训练中的梯度问题，奠定了其在时序处理领域的经典地位\cite{hochreiter1997long}。然而，随着数据规模和任务复杂度的提升，LSTM 在并行计算能力和捕捉超长距离全局依赖方面逐渐显露出瓶颈。随后，基于自注意力机制的 Transformer 架构横空出世，以其卓越的全局上下文捕捉能力和高度并行的计算优势，彻底改变了序列建模的范式\cite{vaswani2017attention}。

随着这两种基础模型的广泛应用，研究人员开始对它们在时序预测中的表现进行深入对比与审视。研究表明，单一架构往往难以在不同复杂度的任务中实现全面领先。例如，在针对金融等特定时序数据的预测中，LSTM 和 Transformer 各有侧重\cite{hollis2018comparison}。近期更有研究指出，Transformer 在处理某些长程时间序列时存在丢失局部时序信息的局限性\cite{alharthi2024xlstmtime}；而在水文等预测任务的基准测试中，LSTM 在捕捉简单局部模式时表现更稳健，而 Transformer 则在处理复杂多元依赖时更具优势\cite{liu2025rnns}。
...
\subsection{国内外研究现状}
近年来，基于 Transformer 的模型在长程时序预测领域发展迅速\cite{su2025systematic}。为了克服单一模型的固有缺陷，融合 LSTM 与 Transformer 的混合架构已经成为国内外学者的研究热点，并在多个跨学科领域取得了优于单一模型的成果。

在国内研究方面，学者们已初步验证了混合模型在金融等高噪声时序场景中的有效性。例如，有研究构建了基于 LSTM-Transformer 的混合模型用于股票价格趋势的预测，有效提升了预测精度\cite{1022075900.nh}。类似地，在白银期货价格预测等复杂金融衍生品研究中，结合两者的混合模型也展现出了强大的特征表征能力和鲁棒性\cite{1025341961.nh}。

在国外研究方面，混合架构的应用场景更为广泛且深入。在金融时间序列预测中，最新的研究提出了稳健的混合深度学习模型，其实际表现显著优于传统的单一网络\cite{kabir2025lstm}。除了金融领域，该类混合模型还被广泛推广至各种复杂的工业与工程系统中。例如，在人体运动能量收集预测\cite{gong2025dynamic}、锂离子电池剩余使用寿命（RUL）的高精度预测\cite{zhao2025correction}以及自然灾害如山洪发生率的预测\cite{zhang2026time}等任务中，动态融合 LSTM 与 Transformer 的模型均被证明能够有效整合局部与全局特征，实现了预测性能的显著跃升。
