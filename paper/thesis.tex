...
\subsection{研究背景与意义}
\begin{itemize}
  \item 传统金融量化谈到深度学习的崛起。
\end{itemize}
\subsection{国内外研究现状}
\subsubsection{传统统计学模型在金融预测中的应用（ARIMA）}
\subsubsection{机器学习模型在金融预测中的应用（XGBoost）}
\subsubsection{深度学习在时间序列预测中的应用（LSTM）}
\subsubsection{Transformer架构在时间序列中的前沿探索}
